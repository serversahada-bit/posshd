...
export default function EditOrderForm() {
  ...
  const [failedProofUrl, setFailedProofUrl] = useState('');
      ...
      .then((loaded: Data) => {
        ...
        const nextItems =
          loaded.items.map((item: any) => {
            ...
            const productList = isGift ? loaded.gifts : loaded.products;
            const matched = productList?.find((p: any) => Number(p.id) === Number(item.product_id));
            const matchedBundle = isBundle ? loaded.bundles?.find((bundle: any) => Number(bundle.id) === Number(item.product_id) || String(bundle.product_name || '').trim().toLowerCase() === String(item.product_name || '').trim().toLowerCase()) : null;
            const bundleWeight = matchedBundle
              ? (matchedBundle.components || []).reduce((sum: number, component: any) => {
                  const componentWeight = productWeightById.get(Number(component.product_id)) ?? 0;
                  return sum + (componentWeight * number(component.qty));
                }, 0)
              : null;
  ...
  const totalWeight = useMemo(
    () => items.reduce((sum, item) => sum + number(item.weight_gram) * number(item.qty), 0),
    [items],
  );
  ...
  const getShippingWeightMultiplier = (courierName: string, tWeight: number) => {
    if (!data) return 1;
    const rule = data.courierWeightRules?.[courierName.toUpperCase()] || {};
    const settings = data.shippingWeightSettings || {};
    ...
    try {
      ...
      Object.entries(json.origins).forEach(([origin, originData]: [string, any]) => {
        ...
        Object.entries(originData.rates || {}).forEach(([courierCode, rate]: [string, any]) => {
          ...
          // Hitung multiplier berat seperti di buat_pesanan
          const multiplier = getShippingWeightMultiplier(courierCode, totalWeight);
          const adjustedPrice = number(rate.price) * multiplier;
      ...
      if (form.courier_id && form.warehouse_id && destinationOrWeightChanged) {
        const activeOption = nextOptions.find(
          (opt) =>
            ...
            String(opt.warehouseId) === String(form.warehouse_id),
        );
        if (activeOption) {
          setForm((prev: Record<string, any>) => ({ ...prev, shipping_cost: activeOption.price }));
        }
      }
    } catch (error) {
      setAvailableCouriers([]);
  ...
  }, [data, form.subdistrict, items.length, items, totalWeight]);
  ...
  const add = (kind: 'product' | 'gift' | 'bundle', rawId: string) => {
        ...
        setItems((prev) => {
          ...
          components.forEach((c: any) => {
            ...
            if (existingIndex >= 0) {
              ...
              // Lookup weight dari data products
              const productData = data.products.find((p: any) => Number(p.id) === Number(c.product_id));
              nextItems.push({
                ...
                weight_gram: number(productData?.weight_gram ?? data.shippingWeightSettings?.default_item_weight_gram ?? 200),
              });
            }
          });
          return nextItems;
    ...
    // Lookup weight_gram dari list produk/gift
    const weightGram = isGift
      ? number(found.weight_gram ?? data.shippingWeightSettings?.default_item_weight_gram ?? 200)
      : number(found.weight_gram ?? data.shippingWeightSettings?.default_item_weight_gram ?? 200);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (requiresNewBankTransferProof && !proof) {
      await Swal.fire('Perlu Bukti Transfer Baru', 'Pesanan ini sebelumnya ditolak oleh FAT. Upload bukti transfer baru sebelum menyimpan ulang.', 'warning');
      return;
    }
                ...
                <div>
                  ...
                  {data?.payment?.payment_status === 'rejected' ? (
                    <p className="mt-2 text-xs font-medium text-red-700">
                      Untuk kirim ulang ke validasi FAT, bukti transfer baru wajib diunggah saat simpan.
                    </p>
                  ) : null}
                </div>
              </div>
                        ...
                        items.map((item, index) => (
                          <tr key={index} className="hover:bg-slate-50/50">
                            <td className="p-4 max-w-[180px]">
                              <div className="font-semibold text-slate-800 break-all">{item.product_name}</div>
                            </td>
                            <td className="p-4">
                ...
                <Field label="Ganti Bukti Pembayaran">
                  ...
                  {currentProofUrl ? (
                    <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <a className="mb-3 inline-block text-xs font-medium text-purple-600 hover:text-purple-700" href={currentProofUrl} target="_blank" rel="noreferrer">
                        Buka gambar penuh
                      </a>
                      <img
                        src={currentProofUrl}
                        alt="Bukti pembayaran saat ini"
                        onError={() => setFailedProofUrl(currentProofUrl)}
                        onLoad={() => setFailedProofUrl((prev) => (prev === currentProofUrl ? '' : prev))}
                        className="max-h-72 w-auto rounded-lg border border-slate-200 bg-white object-contain"
                      />
                      {proofLoadFailed ? (
                        <p className="mt-3 text-xs text-amber-600">
                          File bukti transfer tidak ditemukan di storage lokal. Path tersimpan: {form.payment_proof_url}{requiresNewBankTransferProof ? ' Karena pesanan ini ditolak FAT, unggah bukti transfer baru untuk melanjutkan.' : ''}
                        </p>
                      ) : null}
                    </div>
                  ) : null}
            ...
            <section className={sectionClass}>
              ...
              <div className={`${sectionBodyClass} space-y-4`}>
                ...
                <Field label="Promo">
                  <Select
                    isMulti
                    value={(form.promo_id || '').split(',').filter(Boolean).map((id: string) => {
                      const p = data.promos.find((pr: any) => pr.id.toString() === id.trim());
                      return p ? { value: p.id.toString(), label: p.promo_name } : null;
                    }).filter(Boolean)}
                    onChange={(newVal) => set("promo_id", (newVal as any[]).map(v => v.value).join(','))}
                    options={data.promos.map((item: any) => ({ value: item.id.toString(), label: item.promo_name }))}
