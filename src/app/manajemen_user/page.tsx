...
const availableMenus = [
  ...
  { key: 'data_kostumer', label: 'Data Kostumer' },
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = {
        ...
        role: form.role,
        permissions: form.role === 'admin' ? availableMenus.map((item) => item.key) : form.permissions,
      };
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Kelola data pengguna sistem, username, password, dan role.</p>
        </div>
        <button
          type="button"
