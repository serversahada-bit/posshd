...
const sanitizeSettings = (input?: Partial<WeightSettings>): WeightSettings => ({
  default_item_weight_gram: Math.max(0, readNumber(input?.default_item_weight_gram) ?? DEFAULT_SETTINGS.default_item_weight_gram),
  base_weight_gram: Math.max(1, readNumber(input?.base_weight_gram) ?? DEFAULT_SETTINGS.base_weight_gram),
  extra_weight_step_gram: Math.max(1, readNumber(input?.extra_weight_step_gram) ?? DEFAULT_SETTINGS.extra_weight_step_gram),
  rounding_tolerance_gram: Math.max(0, readNumber(input?.rounding_tolerance_gram) ?? DEFAULT_SETTINGS.rounding_tolerance_gram),
});
...
async function getSettingsPayload() {
  ...
  return {
    ...
    couriers: couriers.map((courier) => ({
      ...
      code: courier.code,
      base_weight_gram: Math.max(1, readNumber(courier.base_weight_gram) ?? settings.base_weight_gram),
      extra_weight_step_gram: Math.max(1, readNumber(courier.extra_weight_step_gram) ?? settings.extra_weight_step_gram),
      rounding_tolerance_gram: Math.max(0, readNumber(courier.rounding_tolerance_gram) ?? settings.rounding_tolerance_gram),
    })),
  };
}
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const rule of courierRules) {
        ...
        await tx.couriers.update({
          where: { id: courierId },
          data: {
            base_weight_gram: Math.max(1, readNumber(rule?.base_weight_gram) ?? settings.base_weight_gram),
            extra_weight_step_gram: Math.max(1, readNumber(rule?.extra_weight_step_gram) ?? settings.extra_weight_step_gram),
            rounding_tolerance_gram: Math.max(0, readNumber(rule?.rounding_tolerance_gram) ?? settings.rounding_tolerance_gram),
          },
        });
      }
    });
