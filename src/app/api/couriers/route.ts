...
export async function POST(request: NextRequest) {
  try {
    ...
    const base_weight_gram = Math.max(1, readNumber(body?.base_weight_gram) ?? 1000);
    const extra_weight_step_gram = Math.max(1, readNumber(body?.extra_weight_step_gram) ?? 1000);
    const rounding_tolerance_gram = Math.max(0, readNumber(body?.rounding_tolerance_gram) ?? 300);
