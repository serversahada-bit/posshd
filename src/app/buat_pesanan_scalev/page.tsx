...
import { Loader2, Plus, Search, ShoppingCart, Trash2, MapPin, Package, Upload, AlertCircle, ChevronDown, ArrowLeft } from 'lucide-react';
...
// Cocokkan nama dari Scalev ke salah satu opsi lokal (advertiser/sumber iklan) tanpa harus sama persis:
// prioritas exact match > salah satu string mengandung yang lain > kemiripan kata (token overlap).
function bestFuzzyMatch(candidates: string[], target: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    if (candidateNorm === targetNorm) return candidate;
    ...
    const substringBonus = (targetNorm.includes(candidateNorm) || candidateNorm.includes(targetNorm)) ? 1 : 0;
    const score = (overlap / candidateTokens.length) + substringBonus;
...
// Kode Sumber Iklan di sistem ini berupa singkatan (LZ, SP, TT, SV, GO, MT) yang tidak bisa
// dicocokkan lewat kemiripan kata biasa terhadap teks Scalev ("GOOGLE - GAMAMILK PLU", dll),
// jadi dipetakan eksplisit ke kata kunci yang mungkin muncul di store/channel/platform Scalev.
const AD_SOURCE_ALIASES: Record<string, string[]> = {
  lz: ['lazada'],
  sp: ['shopee'],
  tt: ['tiktok', 'tiktokshop', 'tik tok'],
  sv: ['snack video', 'snackvideo', 'snack'],
  go: ['google', 'gads'],
  mt: ['meta', 'facebook', 'instagram', 'fb'],
};

function matchAdSourceByAlias(candidates: string[], target: string): string | null {
  const targetNorm = normalizeForMatch(target || '');
  if (!targetNorm) return null;

  for (const candidate of candidates) {
    const key = normalizeForMatch(candidate);
    const aliases = AD_SOURCE_ALIASES[key];
    if (aliases?.some((alias) => targetNorm.includes(alias))) {
      return candidate;
    }
  }
  return null;
...
export default function BuatPesananScalevPage() {
  const { user } = useAuth();
  const searchParams = useSearchParams();
  const scalevOrderIdParam = searchParams.get('scalev_order_id') || '';
  ...
  const [promoIds, setPromoIds] = useState<any[]>([]);
  ...
  // Data order asal dari Scalev (untuk auto-fill & pencocokan alamat)
  const [scalevOrder, setScalevOrder] = useState<any>(null);
  const [scalevLoading, setScalevLoading] = useState(Boolean(scalevOrderIdParam));
  const [scalevError, setScalevError] = useState('');
  const [unmatchedScalevProducts, setUnmatchedScalevProducts] = useState<string[]>([]);
  ...
  // Tarik detail order dari Scalev (kalau halaman dibuka dari tombol "Buat Pesanan" di Data Scalev)
  // lalu auto-fill data pelanggan & alamat. Kecamatan/Kota/Provinsi TIDAK dipilih otomatis —
  // hanya disiapkan opsi pencariannya, karena Scalev tidak menyediakan data kecamatan yang presisi.
  useEffect(() => {
    if (!scalevOrderIdParam) return;

    let cancelled = false;

    (async () => {
      setScalevLoading(true);
      setScalevError('');
      try {
        const res = await fetch(`/api/scalev/orders/${encodeURIComponent(scalevOrderIdParam)}`);
        const json = await res.json();
        ...
        setScalevOrder(order);
        setOrderCode((order.order_id || scalevOrderIdParam).toUpperCase());

        if (order.customer?.name) setCustomerName(order.customer.name);
        if (order.customer?.email) setEmail(order.customer.email);
        if (order.destination_address?.address) setAddress(order.destination_address.address);
        if (order.notes) setComplaint((prev) => prev || order.notes);

        setNotes((prev) => prev || `Order Scalev: ${order.order_id}${order.store?.name ? ' - ' + order.store.name : ''}`);

        if (order.customer?.phone) {
          handleWaCheck(order.customer.phone);
        }

        const destQuery = [order.destination_address?.city, order.destination_address?.province].filter(Boolean).join(', ');
        if (destQuery) {
          handleDestSearch(destQuery);
        }
      } catch (err: any) {
        if (!cancelled) setScalevError(err.message || 'Gagal mengambil data order Scalev');
      } finally {
        if (!cancelled) setScalevLoading(false);
      }
    })();
  ...
  // Cocokkan Nama Advertiser & Sumber Iklan dari data Scalev ke daftar internal kita.
  // Tidak perlu sama persis dengan teks Scalev — dicari kemiripan namanya (fuzzy match).
  useEffect(() => {
    if (!scalevOrder || !data) return;

    if (!advertiserName && scalevOrder.advertiser?.fullname && data.advertisers?.length) {
      const match = bestFuzzyMatch(data.advertisers.map((a: any) => a.name), scalevOrder.advertiser.fullname);
      if (match) setAdvertiserName(match);
    }

    if (!adSource && data.adSources?.length) {
      const candidateText = [scalevOrder.store?.name, scalevOrder.channel_name, scalevOrder.utm_source, scalevOrder.platform].filter(Boolean).join(' ');
      if (candidateText) {
        const names = data.adSources.map((a: any) => a.name);
        const match = matchAdSourceByAlias(names, candidateText) || bestFuzzyMatch(names, candidateText);
        if (match) setAdSource(match);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scalevOrder, data]);

  // Isi keranjang otomatis dari `orderlines` Scalev, dicocokkan ke katalog produk lokal
  // pakai nama (fuzzy match) — bukan sama persis, karena nama produk di Scalev bisa beda penulisan.
  useEffect(() => {
    if (!scalevOrder || !data || cart.length > 0) return;

    const lines: any[] = scalevOrder.orderlines || [];
    if (lines.length === 0) return;

    const productNames = data.products.map((p: any) => p.product_name);
    const newItems: CartLine[] = [];
    const unmatched: string[] = [];

    for (const line of lines) {
      const lineName = line.product_name || '';
      const matchName = bestFuzzyMatch(productNames, lineName);
      const product = matchName ? data.products.find((p: any) => p.product_name === matchName) : null;

      if (!product) {
        unmatched.push(lineName || 'Produk tanpa nama');
        continue;
      }

      const qty = Number(line.quantity) || 1;
      const existingIdx = newItems.findIndex((c) => c.product_id === product.id && !c.is_gift);
      if (existingIdx >= 0) {
    ...
    setUnmatchedScalevProducts(unmatched);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scalevOrder, data]);
  ...
  const getShippingWeightMultiplier = (cName: string, tWeight: number) => {
    ...
    const baseWeight = Math.max(Number(rule.base_weight_gram ?? set.base_weight_gram ?? 1000) || 1000, 1);
    const extraStep = Math.max(Number(rule.extra_weight_step_gram ?? set.extra_weight_step_gram ?? 1000) || 1000, 1);
    const tolerance = Math.max(Number(rule.rounding_tolerance_gram ?? set.rounding_tolerance_gram ?? 0) || 0, 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'free' && !paymentProofFile) {
      return Swal.fire('Error', 'Bukti approval wajib diupload untuk Free / Tanpa Pembayaran', 'error');
    }
    ...
    try {
      ...
      if (json.status === 'success') {
        ...
        Swal.fire('Berhasil', 'Pesanan dari Scalev berhasil dibuat dengan kode: ' + json.order_code, 'success').then(() => {
          window.location.href = '/data_scalev';
        });
      } else {
  ...
  return (
    <div className="w-full px-4 md:px-8 lg:px-12 py-6 space-y-6">
      <div className="mb-6">
        {scalevOrderIdParam ? (
          <Link href="/data_scalev" className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 mb-2">
            <ArrowLeft className="w-3.5 h-3.5" /> Kembali ke Data Scalev
          </Link>
        ) : null}
        <h1 className="text-2xl font-bold text-slate-800">Buat Pesanan Baru (Scalev)</h1>
        <p className="text-sm text-slate-500 mt-1">Masukkan rincian pesanan akuisisi baru di bawah ini. Data pelanggan diisi otomatis dari order Scalev berstatus Draft.</p>
      </div>
        ...
        <div className="mb-2">
          {scalevLoading ? (
            <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-4 py-3">
              <Loader2 className="w-4 h-4 animate-spin" /> Mengambil data order dari Scalev...
            </div>
          ) : scalevError ? (
            <div className="flex items-start gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-4 py-3">
              <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              <span>Gagal mengambil data dari Scalev: {scalevError}. Anda tetap bisa mengisi form ini secara manual.</span>
            </div>
          ) : scalevOrder ? (
            <div className="flex items-start gap-2 text-sm text-purple-700 bg-violet-50 border border-violet-200 rounded-lg px-4 py-3">
              <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              <span>
                Data pelanggan, alamat, &amp; produk diisi otomatis dari Scalev (Order <strong>{scalevOrder.order_id}</strong>{scalevOrder.store?.name ? ` - ${scalevOrder.store.name}` : ''}).
                {' '}Mohon <strong>cocokkan/konfirmasi ulang</strong> Kecamatan / Kota / Provinsi di bawah — Scalev tidak menyediakan data kecamatan.
              </span>
            </div>
          ) : null}
          {!scalevLoading && unmatchedScalevProducts.length > 0 ? (
            <div className="flex items-start gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mt-2">
              <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              <span>
                {unmatchedScalevProducts.length} produk dari Scalev tidak ditemukan di katalog dan <strong>tidak</strong> otomatis ditambahkan: {unmatchedScalevProducts.join(', ')}.
                {' '}Tambahkan manual lewat "+ Tambah Produk" kalau memang diperlukan.
              </span>
            </div>
          ) : null}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-visible">
          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <input required value={orderCode} onChange={e => handleOrderCodeChange(e.target.value)} readOnly={Boolean(scalevOrderIdParam)} type="text" className={`w-full border rounded-lg px-4 py-2.5 focus:ring-1 outline-none text-sm placeholder:text-slate-400 ${orderCodeError ? 'border-red-400 bg-red-50 text-red-600 focus:ring-red-200 focus:border-red-400' : 'border-slate-300 focus:ring-purple-300 focus:border-purple-300'} ${scalevOrderIdParam ? 'bg-slate-50 text-slate-500 cursor-not-allowed' : ''}`} placeholder="Contoh: SCV1234567890" />
              {orderCodeError ? (
                <p className="text-[11px] font-medium mt-1 text-red-500">{orderCodeError}</p>
              ) : scalevOrderIdParam ? (
                <p className="text-[11px] font-medium mt-1 text-slate-400">Otomatis diisi dari Data Scalev.</p>
              ) : (
            ...
            <div>
              ...
              {scalevOrderIdParam && advertiserName && scalevOrder?.advertiser?.fullname ? (
                <p className="text-[11px] font-medium mt-1 text-emerald-600">Dicocokkan otomatis dari Scalev: "{scalevOrder.advertiser.fullname}"</p>
              ) : null}
            </div>
            <div>
              ...
              {scalevOrderIdParam && adSource && scalevOrder?.store?.name ? (
                <p className="text-[11px] font-medium mt-1 text-emerald-600">Dicocokkan otomatis dari Scalev: "{scalevOrder.store.name}"</p>
              ) : null}
            </div>
            ...
            <div className="md:col-span-2">
              ...
              {scalevOrderIdParam && !subdistrict ? (
                <p className="text-[11px] font-medium mt-1 text-amber-600">Pilih salah satu hasil pencarian di atas untuk mencocokkan alamat dari Scalev.</p>
              ) : null}
            </div>
              ...
              <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <Select
                    isMulti
                    value={promoIds}
                    onChange={(newVal) => setPromoIds(newVal as any[])}
                    options={data?.promos.map(p => ({ value: p.id, label: p.promo_name })) || []}
